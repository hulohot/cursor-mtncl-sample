# MTNCL_FullAdder_Stage_tb.py
#
# Author: Claude
# Date: February 25, 2024
#
# Description:
# Testbench for MTNCL_FullAdder_Stage (pipeline stage with full adder)
...
# Test parameters
RUN_DIR = Path('./run')
VECTOR_FILE = Path('./vectors/full_adder_stage_test_vectors.csv')
...
async def cycle_sleep(dut, sleep_time=20):
    """Generate a complete DATA/NULL cycle for pipeline stage"""
    # Set to active mode (allow combinational logic to work)
    dut.sleep.value = 0
    await Timer(sleep_time, units='ns')
    
    # Wait for ko to go low (DATA wavefront complete)
    ...
    # Wait for ko to go high (NULL wavefront complete)
...
@cocotb.test()
async def test_mtncl_full_adder_stage(dut):
    """Test the MTNCL Full Adder Stage with vectors from file"""
    ...
    with open(VECTOR_FILE, mode='r') as f:
        ...
        # Open results file
        with open(RUN_DIR / 'stage_results.csv', mode='w') as results_file:
